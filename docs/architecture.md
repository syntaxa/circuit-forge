...
*   **`Exercise` (Упражнение):** Группы мышц и сложность заданы enum-ами `MuscleGroup` и `Difficulty` (значения: 'ноги' | 'пресс' | 'спина' | 'грудь' | 'руки' | 'плечи' и 'лёгкий' | 'средний' | 'высокий').
    ```typescript
    interface Exercise {
      id: string;
      name: string;
...
*   **`storageService.ts`:** Единый сервис работы с `localStorage`. Управление базой упражнений (CRUD); при первом запуске подставляет «стартовый набор» из `constants.ts` (`SEED_EXERCISES`). CRUD для `Settings`. Сохранение и чтение истории тренировок (`getHistory`, `addLog`, `getLastLog`, `clearLastLog`, `clearHistory`). Ключ для профиля (`cf_profile`) предусмотрен.
...
*   **`ScreenSetup`:** Главный экран с кнопкой старта и отображением выбранных на сегодня групп мышц.
*   **`ScreenSettings`:** Форма настроек, включая выпадающий список для выбора голоса TTS.
*   **`ScreenDatabase`:** Интерфейс для просмотра, добавления, редактирования и удаления упражнений.
