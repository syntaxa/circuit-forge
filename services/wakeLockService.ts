/**
 * Предотвращает блокировку экрана на смартфоне во время тренировки.
 * IMPORTANT: enable() ДОЛЖЕН вызываться из обработчика клика/тапа (user gesture),
 * иначе Wake Lock API и video.play() могут быть отклонены браузером.
 *
 * Использует:
 * 1. Screen Wake Lock API (когда доступен)
 * 2. Fallback: воспроизведение тихого видео в цикле (как NoSleep.js)
 */

const VIDEO_WEBM =
  'data:video/webm;base64,';

const VIDEO_MP4 =
  'data:video/mp4;base64,';

let wakeLockSentinel: WakeLockSentinel | null = null;
let fallbackVideo: HTMLVideoElement | null = null;
let visibilityHandler: (() => void) | null = null;

function hasWakeLockAPI(): boolean {
  return typeof navigator !== 'undefined' && 'wakeLock' in navigator;
}

function reEnable(): void {
  if (document.visibilityState !== 'visible') return;
  if (hasWakeLockAPI() && !wakeLockSentinel) {
    navigator.wakeLock.request('screen').then(
      (sentinel) => {
        wakeLockSentinel = sentinel;
      },
      () => {}
    );
  } else if (fallbackVideo) {
    fallbackVideo.play().catch(() => {});
  }
}

function setupVisibilityListener(): void {
  if (visibilityHandler) return;
  visibilityHandler = () => reEnable();
  document.addEventListener('visibilitychange', visibilityHandler);
  document.addEventListener('fullscreenchange', visibilityHandler);
}

function removeVisibilityListener(): void {
  if (!visibilityHandler) return;
  document.removeEventListener('visibilitychange', visibilityHandler);
  document.removeEventListener('fullscreenchange', visibilityHandler);
  visibilityHandler = null;
}

/**
 * Включает удержание экрана. ДОЛЖЕН вызываться из обработчика клика/тапа (user gesture).
 */
export function enable(): void {
  if (hasWakeLockAPI()) {
    navigator.wakeLock
      .request('screen')
      .then((sentinel) => {
        wakeLockSentinel = sentinel;
        setupVisibilityListener();
      })
      .catch(() => {
        tryVideoFallback();
      });
  } else {
    tryVideoFallback();
  }
}

function tryVideoFallback(): void {
  if (fallbackVideo) {
    fallbackVideo.play().catch(() => {});
    setupVisibilityListener();
    return;
  }
  const video = document.createElement('video');
  video.setAttribute('title', 'NoSleep');
  video.setAttribute('playsinline', '');
  video.setAttribute('muted', '');
  video.setAttribute('loop', '');
  video.style.cssText = 'position:fixed;width:1px;height:1px;opacity:0;pointer-events:none;';
  const srcWebm = document.createElement('source');
  srcWebm.src = VIDEO_WEBM;
  srcWebm.type = 'video/webm';
  video.appendChild(srcWebm);
  const srcMp4 = document.createElement('source');
  srcMp4.src = VIDEO_MP4;
  srcMp4.type = 'video/mp4';
  video.appendChild(srcMp4);
  document.body.appendChild(video);
  fallbackVideo = video;
  video.addEventListener('loadedmetadata', () => {
    if (video.duration <= 1) {
      video.loop = true;
    } else {
      video.addEventListener('timeupdate', () => {
        if (video.currentTime > 0.5) video.currentTime = Math.random();
      });
    }
  });
  video.play().then(
    () => {
      setupVisibilityListener();
    },
    () => {}
  );
}

/**
 * Отключает удержание экрана.
 */
export function disable(): void {
  removeVisibilityListener();
  if (hasWakeLockAPI() && wakeLockSentinel) {
    wakeLockSentinel.release().catch(() => {});
    wakeLockSentinel = null;
  }
  if (fallbackVideo) {
    fallbackVideo.pause();
    fallbackVideo.src = '';
    fallbackVideo.remove();
    fallbackVideo = null;
  }
}
